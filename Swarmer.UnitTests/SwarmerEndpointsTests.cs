...
public sealed class SwarmerEndpointsTests
{
	[Fact]
	public void DdTwitchStreams_NoGameName_ReturnsAllStreams()
	{
		...
		StreamProvider streamProvider = new() { Streams = [stream1, stream2] };
	...
	public void DdTwitchStreams_WithGameName_ReturnsFilteredStreams()
	{
		...
		StreamProvider streamProvider = new() { Streams = streams };

		Stream[]? ddEndpointResult =
			SwarmerEndpoints.DdTwitchStreams(streamProvider, SwarmerEndpoints.DdEndpointParamName);
		Stream[]? hdEndpointResult =
			SwarmerEndpoints.DdTwitchStreams(streamProvider, SwarmerEndpoints.HdEndpointParamName);

		Assert.NotNull(ddEndpointResult);
		Assert.NotNull(hdEndpointResult);

		Assert.All(ddEndpointResult,
			s => s.GameName.Equals(SwarmerEndpoints.DdEndpointParamName, StringComparison.OrdinalIgnoreCase));
		Assert.All(hdEndpointResult,
			s => s.GameName.Equals(SwarmerEndpoints.HdEndpointParamName, StringComparison.OrdinalIgnoreCase));
	}
}
